...
namespace Korot
{
    public partial class frmMakeExt : Form
    {
        ...
        {
            string extManifest = "<?xml version=\"1.0\" encoding=\"UTF-16\"?>" + Environment.NewLine + "<KorotExtension>" + Environment.NewLine +
        ...
        {
            // Read the file
            string ManifestXML = HTAlt.Tools.ReadFile(kemloc, Encoding.Unicode);
            ...
            if (dialogResult == DialogResult.OK)
            {
                try
                {
                    if (dialog.FileName.ToLower().EndsWith(".kef"))
                    {
                        ...
                        Directory.CreateDirectory(workspace);
                        HTAlt.Tools.WriteFile(workspace + "ext.kem", BuildExtManifest(), Encoding.Unicode);
                        foreach (object i in lbSafeName.Items)
                        {
        ...
        {
            string Playlist = HTAlt.Tools.ReadFile(oldKemLoc, Encoding.Unicode);
